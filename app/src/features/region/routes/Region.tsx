    ...
    <div className="container mx-auto px-4">
      {data?.results && data.results.length > 0 ? (
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-16">
          <div className="flex-1 flex flex-col gap-2">
            ...
            <img className="w-14 h-auto object-cover object-center mb-4" src={data?.results[0].origin.flag} alt="flag" />
          </div>
          <p className="flex-[4] whitespace-pre-line">
            {data?.results[0].origin.description}
          </p>
        </div>
      ) : (
      ...
      <div className="flex flex-row gap-8 mt-16">
        {filtersLoading ? (
          ...
          <ArtworksFilterForm filtersToDisplay={['category', 'sub_category']} filters={filters!} defaultValue={artworksFilters} onChange={
            (filters) => {
